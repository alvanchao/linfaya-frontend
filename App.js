// App.js － LINFAYA COUTURE（無「加購修改」版，恢復精簡穩定）
...
function sameLine(i, j){
  return i.id===j.id && i.color===j.color && i.size===j.size;
}
...
function subtotal(){ return state.cart.reduce((s,i)=>s+(i.price||0)*(i.qty||1),0); }
...
function renderCart(){
  ...
  state.cart.forEach((it,idx)=>{
    ...
    row.innerHTML = `
      ...
      <div>
        <div><b>${it.name||''}</b></div>
        <div class="cart-attr" style="color:#8a94a7;font-size:12px">顏色：${it.color||''}｜尺寸：${it.size||''}｜單價：${fmt(it.price||0)}</div>
      ...
      <div class="cart-right" style="text-align:right"><b>${fmt((it.price||0)*(it.qty||1))}</b></div>
    `;
  ...
  extraMount.innerHTML = (PREORDER_MODE && REQUIRE_PREORDER_CHECKBOX && state.cart.length) ? `
...
$('#checkout')?.addEventListener('click', async ()=>{
  ...
  const itemNameRaw = state.cart.map(i=>`${i.name}(${i.color}/${i.size})x${i.qty}`).join('#');
